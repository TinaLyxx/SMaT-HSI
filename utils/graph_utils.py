...
def AcquireData(args):
    ...
    return TR,TE,input,num_classes,input_normalize

def alldata(TE,row, column, TR, num_classes, dim, input_normalize, args):
    allTE = np.ones_like(TE)
    for i in range(12):
        Label_test_all, x_test_position_all, input_normalize = ALL_DataProcee(row, column, TR, allTE, num_classes,
                                                                              dim, input_normalize, args)
    return Label_test_all, x_test_position_all


def ALL_DataProcee(height, width,TR, TE, num_classes,band, input_normalize, args):
    label = TR + TE
    ...
    # padding
    mirror_image = mirror_hsi(height, width, band, input_normalize, patches_real=3)  #

    # obtain train and test features
    x_train_band, x_test_band, x_true_band, x_train_position, x_test_position = \
        train_and_test_data(mirror_image, band, pos_train, pos_test, pos_true, patch_labels, nr, nc, num_patches, args,patches_real=3)

    y_train, y_test, y_true = train_and_test_label(num_train, num_test, num_true, num_classes)
    # -----------------------------------------------------------

    x_test1 = torch.from_numpy(x_test_band).type(torch.FloatTensor)
    y_test = torch.from_numpy(y_test).type(torch.LongTensor)
    x_test_indexs = torch.arange(x_test_position.shape[0])
    Label_test = Data.TensorDataset(x_test1,x_test_indexs, y_test)

    return Label_test,x_test_position, x_test_band.reshape(height, width, band)
...
def DataProcee(height, width,TR, TE, num_classes,band, input_normalize, args):
    ...
    # obtain train and test features
    x_train_band, x_test_band, x_true_band, x_train_position, x_test_position = \
        train_and_test_data(mirror_image, band, pos_train, pos_test, pos_true, patch_labels, nr, nc, num_patches, args, patches_real=args.window_b)
...
#-------------------------------------------------------------------------------
# 汇总训练数据和测试数据
def train_and_test_data(mirror_image, band, train_point, test_point, true_point,  position_matrix, n_r,n_c, num_position, args, patches_real):
...
# -------------------------------------------------------------------------------
# validate model
def valid_epoch(model, valid_loader, x_test_position,  optimizer, pred, args):
    objs = AvgrageMeter()
    top1 = AvgrageMeter()
    tar = np.array([])
    pre = np.array([])
    for batch_idx, (batch_data, batch_data_position,batch_target) in enumerate(valid_loader):
        batch_data = batch_data.to(device)
        batch_data_position1 = np.array(batch_data_position)
        batch_data_position1 = torch.from_numpy(x_test_position[batch_data_position1,:]).type(torch.FloatTensor)
        batch_data_position1 = batch_data_position1.to(device)
        batch_target = batch_target.to(device)
        batch_pred, cross_view = model(batch_data, batch_data_position1)
        ...
        tar = np.append(tar, t.data.cpu().numpy())
        pre = np.append(pre, p.data.cpu().numpy())
        pred[batch_data_position] = p.data.cpu().numpy()+1
...
def output_metric(tar, pre):
    ...
    results["Confusion matrix"] = matrix

    # Compute global accuracy
    total = np.sum(matrix)
    accuracy = sum([matrix[x][x] for x in range(len(matrix))])
    accuracy *= 100 / float(total)

    results["Accuracy"] = accuracy

    # Compute F1 score
    F1scores = np.zeros(len(matrix)+1)
    for i in range(len(matrix)):
        try:
            F1 = 2. * matrix[i, i] / (np.sum(matrix[i, :]) + np.sum(matrix[:, i]))
    ...
    pa = np.trace(matrix) / float(total)
    pe = np.sum(np.sum(matrix, axis=0) * np.sum(matrix, axis=1)) / \
         float(total * total)
    kappa = (pa - pe) / (1 - pe)
    results["Kappa"] = kappa
    # OA, AA_mean, Kappa, AA, F1 = cal_results(matrix)
...
def cal_results(matrix):
    ...
    for i in range(len(matrix)):
        try:
            ...
            F1 = 0.
        F1scores[i] = F1
    return OA, AA_mean, Kappa, AA, F1scores
# -------------------------------------------------------------------------------