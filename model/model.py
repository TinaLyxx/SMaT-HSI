...
from utils.mst import MinimumSpanningTree
...
def seq_generate(x, max_adj=4, if_pos_embed=False):
        B, C, H, W = x.shape
...
class spectral_spatial_block(nn.Module):
    def __init__(self, embed_dim, bi=False, N=8, drop_path=0.0, norm_layer=nn.LayerNorm, cls = True, fu = True, bfs=True):
...
class PatchEmbed_2D(nn.Module):
    ...
    """
    def __init__(self, img_size=(224,224), patch_size=16, in_chans=3, embed_dim=64, norm_layer=None, flatten = True, bfs=True):
        super().__init__()
...
class mamba_SS_model(nn.Module):
    def __init__(self, spa_img_size=(224, 224),spe_img_size=(5,5), spa_patch_size=16, spe_patch_size=2, in_chans=3, hid_chans = 32, embed_dim=128, nclass=10, drop_path=0.0, depth=4, bi=True, 
                 norm_layer=nn.LayerNorm, global_pool=True, cls = True, fu=True, bfs=True):
        super().__init__()
        ...
        self.spa_patch_embed = PatchEmbed_2D(spa_img_size, spa_patch_size, hid_chans, embed_dim,bfs=bfs)
        ...
        self.blocks = nn.ModuleList([
                spectral_spatial_block(embed_dim, bi, N=N, drop_path = drop_path, cls = self.cls, fu = fu, bfs=bfs) for i in range(depth)
                        ])
        self.head = nn.Linear(embed_dim, nclass)
    ...
    def forward_features(self, x):
        ...
        x_spa, sorted_index = self.spa_patch_embed(x_spa)
        x_spa = x_spa + self.spa_pos_embed[:, :-1, :]
        if self.bfs:
            B,L,C = x_spa.shape
            x_spa = x_spa.gather(1, sorted_index.unsqueeze(-1).expand(B, L, C))
        # append cls token
        if self.cls:
